...
public class ReplyCommand<T> {
     ...
     */
    public ReplyCommand(Action execute, Function<T, Boolean> canExecute0) {
        this.execute0 = execute;
     ...
     *                    otherwise would't invoked!
     */
    public ReplyCommand(Consumer<T> execute, Function<T, Boolean> canExecute0) {
        this.execute1 = execute;
        this.canExecute0 = canExecute0;
    }
    ...
    public void execute(T parameter) {
        try {
            if (execute0 != null && canExecute0(parameter)) {
                execute0.run();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void execute1(T parameter) {
        try {
